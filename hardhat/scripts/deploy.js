...
async function main() {
  ...
  writeFileSync("../client4/src/contractData/axonToken.json", JSON.stringify(axonTokenData, null, 2), (err) => {
    if (err) {
      console.error(err);
      return;
    }
  ...
  writeFileSync("../client4/src/contractData/peerReview.json", JSON.stringify(peerReviewData, null, 2), (err) => {
    if (err) {
      console.error(err);
      return;
    }
